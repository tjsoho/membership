...
export async function GET(req: Request) {
  try {
    ...
    let items;
    if (user.isAdmin) {
      // Admin: get all workspace items for the course
      items = await prisma.workspaceItem.findMany({
        where: { courseId },
...
export async function POST(req: Request) {
  try {
    ...
    if (body.type === "MINDMAP") {
      ...
      const isValidSlateContent = body.content.every(
        (node: any) =>
          typeof node === "object" &&
          node.type === "paragraph" &&
          Array.isArray(node.children) &&
          node.children.every(
            (child: any) => typeof child === "object" && "text" in child
          )
      );
      if (!isValidSlateContent) {
        console.log(
          "API POST - Invalid notes content structure:",
          body.content
        );
        return new NextResponse("Invalid notes content structure", {
          status: 400,
        });
      }
    }
    ...
    const contentToSave =
      body.type === "MINDMAP"
        ? {
            ...
            appState: {
              viewBackgroundColor:
                body.content.appState?.viewBackgroundColor || "#ffffff",
              currentItemFontFamily:
                body.content.appState?.currentItemFontFamily || 1,
    ...
    console.log(
      "API POST - Creating workspace item with content:",
      contentToSave
    );
